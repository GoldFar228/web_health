...
interface SearchableFoodInputProps {
  ...
  initialValue?: string;
  minLength?: number; // порог для начала поиска (по умолчанию 4)
  debounceMs?: number; // задержка поиска (по умолчанию 300)
}
...
export const SearchableFoodInput: React.FC<SearchableFoodInputProps> = ({
  ...
  // Дебаунс-поиск с приоритетом: FatSecret → Personal History
  const performSearch = useCallback(async (searchQuery: string) => {
    ...
    try {
      ...
      // Фоллбэк только на историю при ошибке FatSecret
      try {
        const personalResults = await searchPersonalFoodHistory(searchQuery);
  ...
  return (
    <div className="searchable-food-input__wrapper" ref={dropdownRef}>
      ...
      {isOpen && query.length >= minLength && results.length === 0 && !isLoading && (
